...
def getFiles(train, path):
    ...
    for folder in os.listdir(path):
        if(folder.startswith('.')): continue # Ignore the .DS_Stroe
        for files in  os.listdir(os.path.join(path, folder)):
            if(files.startswith('.')): continue # Ignore the .DS_Stroe
            images.append(os.path.join(path, os.path.join(folder, files)))
...
def trainModel(path, no_clusters, kernel):
    ...
    for img_path in tqdm(images):
        class_index = labels[img_path.split('/')[2]]
        # if("city" in img_path):
        #     class_index = 0
...
def testModel(path, kmeans, scale, svm, im_features, no_clusters, kernel):
    ...
    predictions = [labels.get(i) for i in svm.predict(kernel_test)]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_path', action="store", dest="train_path",  default='data/training')
    parser.add_argument('--test_path', action="store", dest="test_path", default='data/testing')
